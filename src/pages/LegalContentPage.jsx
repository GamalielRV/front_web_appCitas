...
import { getPlatformTerms, getPublicTerms, updatePlatformTerms } from '../services/legalContentApi'
...
function LegalContentPage() {
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      ...
      setError('')
      try {
        const legal = canEdit ? await getPlatformTerms() : await getPublicTerms()
        if (!mounted) {
          return
        }
        setForm((prev) => ({
        ...
        if (mounted) {
          setError('No se pudo cargar contenido legal desde API. Mostrando borrador local.')
        }
      } finally {
        if (mounted) {
          setLoading(false)
